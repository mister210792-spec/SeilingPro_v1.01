        ...
        r.elements.forEach((el, idx) => {
            ...
            if (!isExport) {
                if (isMobile) {
                    ...
                    g.addEventListener('touchend', handleElementTouchEnd, { passive: false });
    ...
    if (!isExport) {
        r.points.forEach((p, i) => {
            ...
            if (isMobile) {
                ...
                c.addEventListener('touchend', handlePointTouchEnd, { passive: false });
...
function handleElementTouchEnd(e) {
    ...
    if (touchState.dragElem && !touchState.moved && mobileTool !== 'delete') {
        selectedElement = touchState.dragElem;
        showElementContextMenu(touchState.dragElem);
    }
...
function showElementContextMenu(el) {
    ...
    document.getElementById('menu-edit-length').style.display = hasLength ? 'block' : 'none';
...
function menuRotate() {
    closeElementContextMenu();
    if (selectedElement) {
        let newRot = prompt('Введите угол поворота (градусы):', selectedElement.rotation || 0);
        if (newRot && !isNaN(newRot)) {
            saveState();
            selectedElement.rotation = parseFloat(newRot);
            draw();
        }
    }
}
