...
let selectedElementForRotation = null;
...
function showRotateButton(el) {
    selectedElementForRotation = el;

    const pxX = mmToPx(el.x, 'x');
    const pxY = mmToPx(el.y, 'y');

    rotateBtn.style.left = (pxX + 20) + "px";
    rotateBtn.style.top = (pxY - 20) + "px";
    rotateBtn.style.display = "block";

    deleteBtn.style.left = (pxX - 50) + "px";
    deleteBtn.style.top = (pxY - 20) + "px";
    deleteBtn.style.display = "block";
}


function hideRotateButton() {
    selectedElementForRotation = null;
    rotateBtn.style.display = "none";
    deleteBtn.style.display = "none";
}

rotateBtn.onclick = () => {
    if (!selectedElementForRotation) return;

    selectedElementForRotation.rotation =
        (selectedElementForRotation.rotation || 0) + 15;
...
deleteBtn.onclick = () => {
    ...
    r.elements = r.elements.filter(el => el !== selectedElementForRotation);
...
function draw(isExport = false) {
    ...
    let def = getElementDef(el.subtype);
    let isLinear = def.type === 'linear' || el.type === 'rail';

    // Показываем кнопку для ЛЮБОГО устройства
    if (isLinear) {
        showRotateButton(el);
        return;
    }
...
// --- ФУНКЦИИ УПРАВЛЕНИЯ ПРОЕКТАМИ (ИСПРАВЛЕНО) ---
...
function openProjectsModal() { 
    renderProjectsList();
    document.getElementById('projectsListModal').style.display = 'flex'; 
}

// ЭТОЙ ФУНКЦИИ У ВАС НЕ ХВАТАЛО
...
function saveProjectToStorage() {
    ...
    const projectData = {
        ...
        data: JSON.parse(JSON.stringify(rooms)) // Глубокое копирование комнат
    };

    let allProjects = JSON.parse(localStorage.getItem('ceiling_projects') || '[]');
    allProjects.push(projectData);
    localStorage.setItem('ceiling_projects', JSON.stringify(allProjects));
...
function renderProjectsList() {
    ...
    const allProjects = JSON.parse(localStorage.getItem('ceiling_projects') || '[]');
    const userEmail = currentUser ? currentUser.email : 'guest';
    const userProjects = allProjects.filter(p => p.userId === userEmail);

    if(userProjects.length === 0) {
        container.innerHTML = `
            <div style="text-align:center; padding:50px 20px;">
                <div style="font-size: 40px; margin-bottom: 10px;">📂</div>
                <p style="color:#95a5a6;">У вас пока нет сохраненных проектов</p>
            </div>`;
        return;
    }
    ...
    container.innerHTML = `
        <table class="projects-table">
            ...
            <tbody>
                ${userProjects.reverse().map(p => `
                    <tr>
                        <td>
                            <span class="project-info-main">${p.info.title}</span>
                            <span class="project-info-sub">👤 ${p.info.client || 'Не указан'} ${p.info.phone ? '• ' + p.info.phone : ''}</span>
                        </td>
                        <td><span style="font-size: 13px;">📍 ${p.info.address || '-'}</span></td>
                        <td><span style="font-size: 13px;">📅 ${p.info.date || '-'}</span></td>
                        <td style="text-align: right;">
    <button class="btn-open" onclick="loadProject(${p.id})">Открыть</button>
...
function loadProject(id) {
    if(!confirm("Текущий чертеж будет заменен. Продолжить?")) return;
    const allProjects = JSON.parse(localStorage.getItem('ceiling_projects') || '[]');
