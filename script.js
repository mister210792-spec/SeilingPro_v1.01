...
function linkTelegramAccount() {
  ...
  // Сохраняем UID пользователя для привязки
...
function linkTelegram() {
    ...
    navigator.clipboard.writeText(command).then(() => {
        ...
        // Показываем уведомление, которое само закроется
...
function showPaymentReminder() {
    ...
    `;
    reminder.innerHTML = '⏳ Ожидается оплата PRO. <span style="text-decoration:underline">Оплатить</span>';
    reminder.onclick = () => {
...
function copyToClipboard(text) {
    ...
    navigator.clipboard.writeText(text).then(() => {
        ...
        // Если не удалось скопировать (старые браузеры)
