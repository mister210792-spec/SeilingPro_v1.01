...
// Функции для кнопок зума без ограничения уменьшения
function zoomIn() {
    const rect = svg.getBoundingClientRect();
    const centerX = rect.width / 2;
    const centerY = rect.height / 2;
    
    // Оставляем только ограничение на максимум
...
function zoomOut() {
    ...
    // Убираем ограничение на минимум - можно уменьшать сколько угодно
    let newScale = scale * 0.8; // Просто уменьшаем на 20%
...
function resetView() {
    // Сбрасываем масштаб и позицию к начальным значениям
    scale = 0.18;
    offsetX = 100;
    offsetY = 100;
    updateZoomLevel();
